...
class Message extends Base {
	...
	const sudoIds = (config.SUDO || '').split(',').filter(Boolean);
        this.sudo = [jidNormalizedUser(this.client.user.id), ...sudoIds.map(id => id.replace(/[^0-9]/g, ''))].map(id => `${id}@s.whatsapp.net`);
        this.isSudo = this.sudo.includes(this.sender.replace(/[^0-9]/g, '') + '@s.whatsapp.net');
