...
async function sendUrl(message, message.reply_message) {
if(message.reply_message.type == "stickerMessage") {
	...
	} else if (message.reply_message.type == "imageMessage" || message.reply_message.type == "videoMessage") {
	let p = await message.reply_message.downloadMediaMessage()
let u = await uploadImageToImgur(p)
return message.reply(u)
	} else if (message.reply_message.type == "audioMessage") {
let urvideo = await message.reply_message.downloadMediaMessage()
		await ff(urvideo)
...
module.exports = { wawe, MediaUrls, createInteractiveMessage, AudioMetaData, ffmpeg, cutAudio, cutVideo, toAudio, toPTT, sendUrl };
