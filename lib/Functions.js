...
async function setWarn(message, match, WARN_COUNT) {
    ...
    userWarnCount++;
    await message.client.sendMessage(message.jid, { text: `_User @${userId.split("@")[0]} warned._ \n_Warn Count: ${userWarnCount}._ \n_Reason: ${reason}_`, mentions: [userId] });
...
async function resetWarn(message) {
const userId = message.mention[0] || message.reply_message.sender;
    if (!userId) return message.reply("_Mention or reply to someone_");
...
module.exports = { wawe, MediaUrls, createInteractiveMessage, AudioMetaData, ffmpeg, cutAudio, cutVideo, toAudio, toPTT, sendUrl, setWarn, resetWarn, parsedJid };
